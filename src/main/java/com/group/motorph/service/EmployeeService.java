...
public class EmployeeService {
    ...
    // Check if an employee ID already exists. Used to prevent duplicate IDs.
    public boolean employeeIdExists(String employeeId) {
        return employeeDAO.getEmployeeById(employeeId) != null;
    }

    // Add a new employee with validation. Returns an error message, or null if successful.
    public String addEmployee(Employee employee) {
        if (!InputValidationUtil.isNotBlank(employee.getEmployeeId())) {
            return "Employee ID is required.";
        }
        if (employeeIdExists(employee.getEmployeeId())) {
            return "Employee ID " + employee.getEmployeeId() + " already exists.";
        }
        if (!InputValidationUtil.isNotBlank(employee.getFirstName())) {
            return "First name is required.";
        }
        if (!InputValidationUtil.isNotBlank(employee.getLastName())) {
    ...
    // Update an existing employee. Returns an error message, or null if successful.
    ...
    // Delete employee by ID. Returns an error message, or null if successful.
