...
/**
 * Payslips panel – shows the logged-in user's payslips. Columns: Payslip ID | Month | Year | Actions ("View Details" button per row).
 */
public class EmployeePayslipsPanel extends JPanel {
    ...
    private void build() {
        ...
        monthCombo = new JComboBox<>(mOpts.toArray(String[]::new));
        ...
        yearCombo = new JComboBox<>(yOpts.toArray(String[]::new));
        ...
        // Table (Payslip ID | Month | Year | Actions)
        tableModel = new DefaultTableModel(
                new String[]{"Payslip ID", "Month", "Year", "Action"}, 0) {
            @Override
        ...
        table.getColumnModel().getColumn(ACT_COL).setHeaderRenderer(UITheme.centeredHeaderRenderer());
        UITheme.setActionColumns(table, ACT_COL);
        ...
        table.addMouseListener(new java.awt.event.MouseAdapter() {
            ...
            public void mouseClicked(java.awt.event.MouseEvent e) {
                ...
                if (col == ACT_COL && row >= 0 && row < filteredRecords.size()) {
                    ...
                    PayslipDialog.show(rec, employee);
                }
            }
        });

        // Card assembly
        RoundedPanel card = new RoundedPanel(new BorderLayout(0, 10), 12, UITheme.PANEL_BORDER);
    ...
    private void refreshTable() {
        ...
        filteredRecords = allRecords.stream()
                ...
                .sorted(Comparator.<PayrollRecord, Integer>comparing(PayrollRecord::getYear).reversed()
                        .thenComparing(Comparator.comparingInt(PayrollRecord::getMonth).reversed()))
                .collect(Collectors.toList());

        String empId = currentUser.getEmployeeId() != null ? currentUser.getEmployeeId().trim() : "";
        for (PayrollRecord rec : filteredRecords) {
            String payslipId = (rec.getPayslipId() != null && !rec.getPayslipId().isEmpty())
                    ? rec.getPayslipId()
                    : String.format("PS-%d-%02d-%s", rec.getYear(), rec.getMonth(), empId);
    ...
    private static class ButtonCellRenderer implements TableCellRenderer {
        ...
        ButtonCellRenderer(String label) {
            btn = UITheme.tableActionButton(label);
            btn.setOpaque(false);
            cell.add(btn);
        }
        ...
        public Component getTableCellRendererComponent(
            ...
            cell.setBackground(UITheme.rowBackground(t, row, sel));
            return cell;
        }
    }
