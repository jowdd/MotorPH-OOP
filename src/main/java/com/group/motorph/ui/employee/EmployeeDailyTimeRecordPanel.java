...
/**
 * Displays the logged-in user's own Daily Time Record. Summary bar shows total
 * Worked Hours, Over Time and Late.
 */
public class EmployeeDailyTimeRecordPanel extends JPanel {
    ...
    private void build() {
        ...
        monthOpts.add("All");
        for (int m : monthsSet) {
            monthOpts.add(MONTH_NAMES[m - 1]);
        }
        monthCombo = new JComboBox<>(monthOpts.toArray(String[]::new));
        ...
        yearOpts.add("All");
        for (int y : yearsSet) {
            yearOpts.add(String.valueOf(y));
        }
        yearCombo = new JComboBox<>(yearOpts.toArray(String[]::new));
        yearCombo.setFont(UITheme.FONT_BODY);

        // Filter row
        Employee emp = new EmployeeService().getEmployeeById(empId);
        String fullName = (emp != null) ? emp.getFullName() : currentUser.getUsername();
        ...
        JPanel filterComboPanel = new JPanel(new FlowLayout(FlowLayout.RIGHT, 6, 0));
        filterComboPanel.setBackground(UITheme.PANEL_BG);
        filterComboPanel.add(cycleLbl);
        filterComboPanel.add(monthCombo);
        filterComboPanel.add(yearCombo);

        JPanel filterRow = new JPanel(new BorderLayout());
        filterRow.setBackground(UITheme.PANEL_BG);
        filterRow.add(nameLbl, BorderLayout.WEST);
        filterRow.add(filterComboPanel, BorderLayout.EAST);

        // Summary value labels
        workingHrsLbl = boldLabel("0 Mins");
        overtimeLbl = boldLabel("0 Mins");
        lateLbl = boldLabel("0 Mins");
        workingHrsLbl.setForeground(UITheme.PRIMARY);
        overtimeLbl.setForeground(UITheme.SUCCESS);
        lateLbl.setForeground(UITheme.WARNING);
        ...
        tableModel = new DefaultTableModel(
                new String[]{"Date", "Day", "Log In", "Log Out", "Lates", "Over Time"}, 0) {
            @Override
        ...
        // Lates column: WARNING color
        table.getColumnModel().getColumn(4).setCellRenderer(new DefaultTableCellRenderer() {
            @Override
            public java.awt.Component getTableCellRendererComponent(
                    JTable t, Object v, boolean sel, boolean foc, int row, int col) {
                super.getTableCellRendererComponent(t, v, sel, foc, row, col);
                setBackground(UITheme.rowBackground(t, row, sel));
                if (!sel) {
        ...
        // Over Time column: SUCCESS color
    ...
    /**
     * Rebuilds the attendance table and recalculates the summary metrics for
     * the selected month/year filter.
     *
     * The lateness, payable regular time, and overtime values intentionally
     * mirror the same business-hour assumptions used by payroll so employees
     * see numbers that line up with later payslip calculations.
     */
    private void refreshTable() {
        ...
        if (allYears) {
            ...
            int parsed = -1;
            try {
                parsed = Integer.parseInt(selYear);
        ...
        for (AttendanceRecord rec : rows) {
            ...
            String dayStr = rec.getDate().getDayOfWeek().getDisplayName(TextStyle.FULL, Locale.ENGLISH);
            String inStr = "--:--";
            String outStr = "--:--";
            String latesStr = "";
            String otStr = "";
            ...
            if (rec.getClockIn() != null) {
                inStr = rec.getClockIn().format(TIME_12H);
            }
            if (rec.getClockOut() != null) {
                outStr = rec.getClockOut().format(TIME_12H);
            }
            ...
            if (rec.getClockIn() != null && rec.getClockOut() != null) {
                ...
                // Per-row late minutes
                if (isLate) {
                    long rowLateMins = Duration.between(BUSINESS_START, ci).toMinutes();
                    totalLateMins += rowLateMins;
                    latesStr = formatDuration(rowLateMins);
                }
                ...
                if (co.isAfter(BUSINESS_END)) {
                    long rowOtMins = Duration.between(BUSINESS_END, co).toMinutes();
                    totalOtMins += rowOtMins;
                    otStr = formatDuration(rowOtMins);
                }
            }

            tableModel.addRow(new Object[]{dateStr, dayStr, inStr, outStr, latesStr, otStr});
        }

        workingHrsLbl.setText(formatDuration(totalWorkMins));
        overtimeLbl.setText(formatDuration(totalOtMins));
        lateLbl.setText(formatDuration(totalLateMins));
    }

    // Helpers
    /**
     * Formats total minutes into a human-readable duration string. e.g. 34 -
     * "34 Minutes", 90 - "1 Hour and 30 Minutes", 120 - "2 Hours"
     */
    static String formatDuration(long totalMinutes) {
        ...
        String hPart = hours + " Hrs";
        if (mins == 0) {
            return hPart;
        }
        return hPart + " " + mins + " Mins";
    }
